<?php 
	...
	class Roles extends Controllers{
		...
		public function Roles()
		{
			...
			$data['page_title'] = "Roles Usuario <small> Yakkar Eventos </small>";
		...
		public function getSelectRoles()
		{
			...
			if(count($arrData) > 0 ){
				for ($i=0; $i < count($arrData); $i++) { 
					if($arrData[$i]['status'] == 1 ){ //TODO: validando que el estado del rol se encuentre activo
		...
		public function setRol(){
			...
			if($request_rol > 0 )
			{
               ...
               if ($request_rol === 'exist') {
                  $arrResponse = array('status' => false, 'msg' => 'Imposible Actualizar, Ya! Existe');
               }else{
